...
class Talk extends React.Component {
  ...
  gettalklike = () => {
    storage
      ...
      .then((res) => {
        ...
        fetch(
          'http://47.93.233.220:8099/getuserlike?userid=' +
            res.userid +
            '&&type=talk',
        )
          .then((res) => {
  ...
  gettalkintalk = (commentid) => {
    fetch('http://47.93.233.220:8099/gettalkintalk?commentid=' + commentid)
      .then((res) => {
        return res.json();
      })
      .then((res) => {
    ...
    storage
      ...
      .then((res) => {
        if (this.judgelike(talkid) != true)
          fetch(
            'http://47.93.233.220:8099/talklike?talkid=' +
              talkid +
              '&&userid=' +
              res.userid +
              '&&operate=sure',
    ...
    storage
      ...
      .catch((err) => {
        this.setState({
          user: {
            icon:
              'https://iknow-pic.cdn.bcebos.com/bd3eb13533fa828b84b228d6f31f4134970a5a75?x-bce-process=image/resize,m_lfit,w_600,h_800,limit_1/quality,q_85',
          },
        });
      });
    fetch(
      'http://47.93.233.220:8099/gettalk?commentid=' +
      ...
      .then((res) => {
        ...
        if (this.state.maintalk != '') {
          if (this.state.istalkin == false) {
            Keyboard.dismiss();
            fetch(
              'http://47.93.233.220:8099/inserttalk?commentid=' +
                this.props.route.params.commentid +
                '&&userid=' +
                res.userid +
                '&&maintalk=' +
  ...
  listfilter = (item, talkid) => {
    if (item.talkid == talkid) return true;
    else return false;
  };
  checkbeing = (talkid) => {
    let last = false;
          ...
          <View style={{height: 40, flexDirection: 'row'}}>
            ...
            <TouchableHighlight
              ...
              onPress={() => {
                //   this.gettalklike()
                // alert("点赞")
                item.likecount = this.judgelike(item.talkid)
                  ? item.likecount - 1
                  : item.likecount + 1;
              ...
              <View style={{flexDirection: 'row'}}>
                {!this.judgelike(item.talkid) ? (
                  <MaterialCommunityIcons
                    ...
                    color={'grey'}
                    size={20}
                    style={{marginLeft: 5, marginTop: 10, marginRight: 3}}
                  />
                ) : (
                  <MaterialCommunityIcons
                    name="thumb-up"
                ...
                <TouchableHighlight
                  ...
                  onPress={() => {
                    this.props.navigation.navigate('所有回复', {
                      zz: item,
                      ll: this.state.talkintalk
                        .filter((zzz) => this.listfilter(zzz, item.talkid))
                        .reverse(),
                      refrush: () => this.gettalkintalk(ddd.commentid),
                    });
                  }}
                  style={{
                    marginHorizontal: 50,
                  ...
                  <View>
                    ...
                    />
                    {this.state.talkintalk.filter((zzz) =>
                      this.listfilter(zzz, item.talkid),
                    ).length >= 3 ? (
      ...
      <View style={{flex: 1}}>
        ...
        <View
          ...
          <View style={{flex: 1, height: 50, backgroundColor: 'white'}}>
            <Text
              style={{textAlign: 'center', height: 50, lineHeight: 50}}
