...
class Commentlist extends React.Component {
  ...
  getcommentlike = () => {
    storage
      ...
      .then((res) => {
        ...
        fetch('http://47.93.233.220:8099/getuserlike?userid=' + res.userid)
          .then((res) => {
            return res.json();
          })
          .then((res) => {
  ...
  // 获取列表数据，进行列表渲染
  componentDidMount = () => {
    fetch('http://47.93.233.220:8099/getcomments')
      .then((res) => {
        return res.json();
      })
      .then((res) => {
    ...
    storage
      ...
      .then((res) => {
        if (this.judgelike(commentid) != true)
          fetch(
            'http://47.93.233.220:8099/commentlike?commentid=' +
              commentid +
              '&&userid=' +
              res.userid +
              '&&operate=sure',
    ...
    storage
      ...
      .then((res) => {
        ...
        if (this.state.textvalue == '')
          ...
          fetch(
            'http://47.93.233.220:8099/insertcomments?main=' +
        ...
        <View style={styles.item}>
          ...
          <View
            ...
            <TouchableHighlight
              ...
              onPress={() => {
                item.likecount = this.judgelike(item.commentid)
                  ? item.likecount - 1
                  : item.likecount + 1;
              ...
              <View style={{flexDirection: 'row'}}>
                ...
                <Text style={{lineHeight: 30, color: 'grey', fontSize: 12}}>
                  {item.likecount}
                </Text>
              </View>
            </TouchableHighlight>
            ...
            <TouchableHighlight
              activeOpacity={0.6}
              underlayColor="#DDDDDD"
              onPress={() => this.props.navigation.navigate('评论', item)}
              style={{
                flex: 1,
                borderBottomRightRadius: 20,
                alignItems: 'center',
      ...
      <SafeAreaView style={styles.container}>
        ...
        {this.state.hideadd == true ? (
          <TouchableHighlight
            activeOpacity={0.6}
            underlayColor="#DDDDDD"
            onPress={() => this.setState({modalhide: true, hideadd: false})}
            style={{
              width: 50,
              height: 50,
              borderRadius: 25,
        ...
        <Modal
          ...
          <View
            ...
            <View
              ...
              <Text style={{height: 20, textAlign: 'center', marginTop: 10}}>
                发表言论
              </Text>
              <TextInput
                style={{
